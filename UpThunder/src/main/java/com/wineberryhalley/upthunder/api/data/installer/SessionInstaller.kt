...
class SessionInstaller(context: Context) : InstallerBase(context) {
    ...
    private fun xInstall(packageName: String, uriList: List<Uri>) {
        ...
        //    Log.i("Writing splits to session for $packageName")
          ...
          //  Log.i("Starting install session for $packageName "+(pendingIntent.intentSender.creatorUid))
            session.commit(pendingIntent.intentSender)
            session.close()
         //   Log.i("Closing install session for $packageName")
        } catch (e: Exception) {
            session.abandon()
            removeFromInstallQueue(packageName)
            android.util.Log.e("MAIN", "xInstall: "+e.message )
            postError(
                packageName,
                e.localizedMessage,
